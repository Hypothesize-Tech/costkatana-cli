...
export function projectCommand(program: Command) {
  ...
  projectGroup
    ...
    .option('-p, --period <period>', 'Budget period (monthly, quarterly, yearly)', 'monthly')
...
async function handleProject(_options: any) {
  ...
  console.log(chalk.white('  costkatana project create <name>     Create a new project'));
  console.log(chalk.white('  costkatana project list              List all projects'));
  console.log(chalk.white('  costkatana project switch <name>     Switch to a project'));
  
  console.log(chalk.gray('\nExamples:'));
  console.log(chalk.white('  costkatana project create my-ai-project'));
  console.log(chalk.white('  costkatana project create impact.p3m --budget 1000'));
...
function displayCreateProjectResult(result: any, options: any) {
  ...
  if (format === 'json') {
    ...
    console.log('Project ID,Name,Description,Budget,Period,Currency,Created At');
    console.log(`"${result._id}","${result.name}","${result.description || ''}","${result.budget?.amount || 0}","${result.budget?.period || ''}","${result.budget?.currency || 'USD'}","${result.createdAt}"`);
    return;
  }
  ...
  if (result.budget) {
    console.log(chalk.white('💰 Budget:'), chalk.green(`${result.budget.currency} ${result.budget.amount.toLocaleString()}`));
  ...
  console.log(chalk.white('📅 Created:'), chalk.cyan(new Date(result.createdAt).toLocaleString()));
  
  console.log(chalk.gray('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
  console.log(chalk.yellow('💡 Next steps:'));
  console.log(chalk.white('  • Switch to this project: costkatana project switch ' + result.name));
  console.log(chalk.white('  • Track usage: costkatana track --project ' + result.name));
  console.log(chalk.white('  • View analytics: costkatana analytics --project ' + result.name));
}
...
function displayProjectsList(projects: any[], options: any) {
  ...
  if (format === 'json') {
    ...
    console.log('Project ID,Name,Description,Budget,Spending,Usage %,Created At');
    projects.forEach(project => {
      const budget = project.budget?.amount || 0;
      const spending = project.spending?.current || 0;
      const usagePercent = budget > 0 ? ((spending / budget) * 100).toFixed(1) : '0.0';
      console.log(`"${project._id}","${project.name}","${project.description || ''}","${budget}","${spending}","${usagePercent}","${project.createdAt}"`);
    });
    return;
  }
  ...
  projects.forEach((project, index) => {
    ...
    if (project.description) {
      console.log(chalk.white('   📄 Description:'), chalk.cyan(project.description));
    }
    
    console.log(chalk.white('   💰 Budget:'), chalk.green(`${project.budget?.currency || 'USD'} ${budget.toLocaleString()}`));
    console.log(chalk.white('   📊 Spending:'), chalk.cyan(`$${spending.toFixed(2)}`));
    console.log(chalk.white('   📈 Usage:'), statusColor(`${usagePercent.toFixed(1)}%`));
    
    if (project.usage) {
      console.log(chalk.white('   🔢 Requests:'), chalk.cyan(project.usage.totalRequests?.toLocaleString() || '0'));
      console.log(chalk.white('   🧠 Tokens:'), chalk.cyan(project.usage.totalTokens?.toLocaleString() || '0'));
    }
    
    console.log(chalk.white('   📅 Created:'), chalk.cyan(new Date(project.createdAt).toLocaleDateString()));
  });

  console.log(chalk.gray('\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
  console.log(chalk.yellow('💡 Commands:'));
  console.log(chalk.white('  • Switch project: costkatana project switch <name>'));
  console.log(chalk.white('  • Create project: costkatana project create <name>'));
  console.log(chalk.white('  • View analytics: costkatana analytics --project <name>'));
}
...
function displaySwitchProjectResult(project: any, options: any) {
  ...
  if (format === 'json') {
    ...
    const spending = project.spending?.current || 0;
    const usagePercent = budget > 0 ? ((spending / budget) * 100).toFixed(1) : '0.0';
    console.log(`"${project._id}","${project.name}","${project.description || ''}","${budget}","${spending}","${usagePercent}"`);
    return;
  }
  ...
  console.log(chalk.white('💰 Budget:'), chalk.green(`${project.budget?.currency || 'USD'} ${budget.toLocaleString()}`));
  console.log(chalk.white('📊 Spending:'), chalk.cyan(`$${spending.toFixed(2)}`));
  console.log(chalk.white('📈 Usage:'), chalk.yellow(`${usagePercent.toFixed(1)}%`));
  ...
  console.log(chalk.gray('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
  console.log(chalk.yellow('💡 Next steps:'));
  console.log(chalk.white('  • Track usage: costkatana track --project ' + project.name));
  console.log(chalk.white('  • View analytics: costkatana analytics --project ' + project.name));
  console.log(chalk.white('  • Check budget: costkatana budget --project ' + project.name));
}
