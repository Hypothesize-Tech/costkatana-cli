...
async function handleRetryLog(options: any) {
  ...
  console.log(chalk.white('  costkatana retry-log range                    Show retry logs by time range'));
  console.log(chalk.white('  costkatana retry-log id <requestId>           Show retry log for specific request'));
  console.log(chalk.white('  costkatana retry-log failure <failureType>    Show retry logs by failure type'));
  console.log(chalk.white('  costkatana retry-log model <modelName>        Show retry logs by model'));
  console.log(chalk.white('  costkatana retry-log stats                    Show retry statistics'));
...
async function getRetryLogsByRange(range: string, options: any) {
  ...
  try {
    ...
    const response = await axios.get(`${baseUrl}/api/retry-log/range?${params}`, {
      headers: {
...
function displayRetryLogs(retryLogs: any, options: any) {
  ...
  console.log(chalk.white('Total Retries:'), chalk.cyan(retryLogs.summary.totalRetries.toLocaleString()));
  console.log(chalk.white('Successful Retries:'), chalk.green(retryLogs.summary.successfulRetries.toLocaleString()));
  console.log(chalk.white('Failed Retries:'), chalk.red(retryLogs.summary.failedRetries.toLocaleString()));
  console.log(chalk.white('Success Rate:'), chalk.green(`${(retryLogs.summary.successRate * 100).toFixed(1)}%`));
  console.log(chalk.white('Average Retry Attempts:'), chalk.cyan(retryLogs.summary.avgRetryAttempts.toFixed(1)));

  // Failure Type Breakdown
  if (retryLogs.failureBreakdown) {
    console.log(chalk.yellow.bold('\n🚨 Failure Type Breakdown'));
    console.log(chalk.gray('─'.repeat(50)));
    Object.entries(retryLogs.failureBreakdown).forEach(([type, data]: [string, any]) => {
      const color = data.count > 0 ? chalk.red : chalk.gray;
      console.log(chalk.white(`${type}:`), color(`${data.count} (${(data.percentage * 100).toFixed(1)}%)`));
      if (data.avgRetries) {
  ...
  if (retryLogs.logs && retryLogs.logs.length > 0) {
    ...
    retryLogs.logs.forEach((log: any, index: number) => {
      ...
      // Failure information
      const failureColor = log.failureCause === 'timeout' ? chalk.yellow : 
                          log.failureCause === 'rate-limit' ? chalk.magenta : chalk.red;
      console.log(chalk.white('   🚨 Failure:'), failureColor(log.failureCause));
      
      // Retry attempts
      console.log(chalk.white('   🔄 Attempts:'), chalk.cyan(log.retryAttempts));
      console.log(chalk.white('   🎯 Strategy:'), chalk.cyan(log.strategy));
      
      // Final outcome
      const outcomeColor = log.finalOutcome === 'success' ? chalk.green : chalk.red;
      console.log(chalk.white('   📊 Outcome:'), outcomeColor(log.finalOutcome));
      
      // Model and timestamp
      console.log(chalk.white('   🤖 Model:'), chalk.cyan(log.model));
      console.log(chalk.white('   ⏰ Time:'), chalk.cyan(new Date(log.timestamp).toLocaleString()));
      ...
      if (options.verbose && log.details) {
        ...
        console.log(chalk.gray(`     Response Time: ${log.details.responseTime}ms`));
        console.log(chalk.gray(`     Retry Delay: ${log.details.retryDelay}ms`));
      }
    });
  } else {
...
function displayRetryLogDetail(retryLog: any, options: any) {
  ...
  console.log(chalk.white('Initial Failure:'), chalk.red(retryLog.initialFailure));
  console.log(chalk.white('Final Outcome:'), retryLog.finalOutcome === 'success' ? chalk.green(retryLog.finalOutcome) : chalk.red(retryLog.finalOutcome));
  console.log(chalk.white('Total Attempts:'), chalk.cyan(retryLog.totalAttempts));
  console.log(chalk.white('Total Duration:'), chalk.cyan(`${retryLog.totalDuration}ms`));
  ...
  retryLog.attempts.forEach((attempt: any, index: number) => {
    ...
    const statusColor = attempt.status === 'success' ? chalk.green : 
                       attempt.status === 'retry' ? chalk.yellow : chalk.red;
    console.log(chalk.white('   Status:'), statusColor(attempt.status));
    console.log(chalk.white('   Time:'), chalk.cyan(new Date(attempt.timestamp).toLocaleString()));
    console.log(chalk.white('   Response Time:'), chalk.cyan(`${attempt.responseTime}ms`));
    ...
    if (attempt.retryDelay) {
      console.log(chalk.white('   Retry Delay:'), chalk.cyan(`${attempt.retryDelay}ms`));
    }
  ...
  if (retryLog.performance) {
    console.log(chalk.yellow.bold('\n📊 Performance Analysis'));
    console.log(chalk.gray('─'.repeat(50)));
    console.log(chalk.white('Average Response Time:'), chalk.cyan(`${retryLog.performance.avgResponseTime}ms`));
    console.log(chalk.white('Total Cost:'), chalk.green(`$${retryLog.performance.totalCost.toFixed(4)}`));
    console.log(chalk.white('Cost per Attempt:'), chalk.cyan(`$${retryLog.performance.costPerAttempt.toFixed(4)}`));
    console.log(chalk.white('Success Rate:'), chalk.green(`${(retryLog.performance.successRate * 100).toFixed(1)}%`));
  }
...
async function getRetryLogsByFailureType(failureType: string, range: string, options: any) {
  ...
  try {
    ...
    const response = await axios.get(`${baseUrl}/api/retry-log/failure-type?${params}`, {
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
...
async function getRetryLogsByModel(modelName: string, range: string, options: any) {
  ...
  try {
    ...
    const response = await axios.get(`${baseUrl}/api/retry-log/model?${params}`, {
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
...
async function getRetryLogStats(range: string, options: any) {
  ...
  try {
    ...
    const response = await axios.get(`${baseUrl}/api/retry-log/stats?${params}`, {
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
...
function displayRetryLogStats(stats: any, options: any) {
  ...
  console.log(chalk.yellow.bold('\n📈 Overall Statistics'));
  console.log(chalk.gray('─'.repeat(50)));
  console.log(chalk.white('Total Requests:'), chalk.cyan(stats.summary.totalRequests.toLocaleString()));
  console.log(chalk.white('Retried Requests:'), chalk.yellow(stats.summary.retriedRequests.toLocaleString()));
  console.log(chalk.white('Retry Rate:'), chalk.cyan(`${(stats.summary.retryRate * 100).toFixed(1)}%`));
  console.log(chalk.white('Successful Retries:'), chalk.green(stats.summary.successfulRetries.toLocaleString()));
  console.log(chalk.white('Failed Retries:'), chalk.red(stats.summary.failedRetries.toLocaleString()));
  console.log(chalk.white('Retry Success Rate:'), chalk.green(`${(stats.summary.retrySuccessRate * 100).toFixed(1)}%`));
  console.log(chalk.white('Average Retry Attempts:'), chalk.cyan(stats.summary.avgRetryAttempts.toFixed(1)));

  // Failure Type Analysis
  if (stats.failureTypes) {
    console.log(chalk.yellow.bold('\n🚨 Failure Type Analysis'));
    console.log(chalk.gray('─'.repeat(50)));
    Object.entries(stats.failureTypes).forEach(([type, data]: [string, any]) => {
      console.log(chalk.white(`\n${type}:`));
      ...
      console.log(chalk.gray(`  Avg Retries: ${data.avgRetries.toFixed(1)}`));
      console.log(chalk.gray(`  Avg Response Time: ${data.avgResponseTime}ms`));
    });
  }
  ...
  if (stats.modelPerformance) {
    console.log(chalk.yellow.bold('\n🤖 Model Performance'));
    console.log(chalk.gray('─'.repeat(50)));
    Object.entries(stats.modelPerformance).forEach(([model, data]: [string, any]) => {
      console.log(chalk.white(`\n${model}:`));
      console.log(chalk.gray(`  Retry Rate: ${(data.retryRate * 100).toFixed(1)}%`));
      console.log(chalk.gray(`  Success Rate: ${(data.successRate * 100).toFixed(1)}%`));
      console.log(chalk.gray(`  Avg Retries: ${data.avgRetries.toFixed(1)}`));
      console.log(chalk.gray(`  Avg Response Time: ${data.avgResponseTime}ms`));
    });
  ...
  if (stats.timePatterns) {
    console.log(chalk.yellow.bold('\n⏰ Time-based Patterns'));
    console.log(chalk.gray('─'.repeat(50)));
    console.log(chalk.white('Peak Retry Hours:'), chalk.cyan(stats.timePatterns.peakHours.join(', ')));
    console.log(chalk.white('Lowest Retry Hours:'), chalk.cyan(stats.timePatterns.lowestHours.join(', ')));
    console.log(chalk.white('Weekly Pattern:'), chalk.cyan(stats.timePatterns.weeklyPattern));
  ...
  if (stats.costImpact) {
    console.log(chalk.yellow.bold('\n💰 Cost Impact'));
    console.log(chalk.gray('─'.repeat(50)));
    console.log(chalk.white('Additional Cost from Retries:'), chalk.red(`$${stats.costImpact.additionalCost.toFixed(2)}`));
    console.log(chalk.white('Cost Increase:'), chalk.red(`${(stats.costImpact.costIncrease * 100).toFixed(1)}%`));
    console.log(chalk.white('Average Cost per Retry:'), chalk.cyan(`$${stats.costImpact.avgCostPerRetry.toFixed(4)}`));
  }
