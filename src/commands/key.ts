...
export function keyCommand(program: Command) {
  ...
  keyGroup
    ...
    .option('-p, --permissions <permissions>', 'Comma-separated permissions (read,write,admin)', 'read')
    .option('--format <format>', 'Output format (table, json, csv)', 'table')
    .option('--export <path>', 'Export key data to file')
...
async function handleKey(options: any) {
  ...
  console.log(chalk.white('  costkatana key create <name>     Create a new API key'));
  console.log(chalk.white('  costkatana key list              List all API keys'));
  console.log(chalk.white('  costkatana key regenerate <id>   Regenerate an API key'));
  console.log(chalk.white('  costkatana key deactivate <id>   Deactivate an API key'));
  
  console.log(chalk.gray('\nExamples:'));
  console.log(chalk.white('  costkatana key create dev-token --budget 100000 --ttl 30d'));
  console.log(chalk.white('  costkatana key create prod-key --permissions read,write'));
...
async function collectKeyData(name: string, options: any) {
  ...
  if (options.permissions) {
    data.permissions = options.permissions.split(',').map((p: string) => p.trim());
  } else {
...
function displayCreateKeyResult(result: any, options: any) {
  ...
  if (format === 'json') {
    ...
    console.log(`"${result.id}","${result.name}","${result.key}","${result.permissions?.join(',') || ''}","${result.created}","${expiresAt}"`);
    return;
  }
  ...
  if (result.permissions && result.permissions.length > 0) {
    console.log(chalk.white('🔓 Permissions:'), chalk.cyan(result.permissions.join(', ')));
  }
  
  if (result.budget) {
    console.log(chalk.white('💰 Budget:'), chalk.green(`${result.budget.toLocaleString()} tokens`));
  }
  
  console.log(chalk.white('📅 Created:'), chalk.cyan(new Date(result.created).toLocaleString()));
  
  if (result.expiresAt) {
    console.log(chalk.white('⏰ Expires:'), chalk.yellow(new Date(result.expiresAt).toLocaleString()));
  } else {
  ...
  console.log(chalk.white('  • Header format:'), chalk.cyan(`X-API-Key: ${result.key}`));
...
function displayKeysList(keys: any[], options: any) {
  ...
  if (format === 'json') {
    ...
    console.log('Key ID,Name,Key Preview,Status,Permissions,Created At,Last Used,Expires At');
    keys.forEach(key => {
      const expiresAt = key.expiresAt || 'Never';
      const lastUsed = key.lastUsed || 'Never';
      console.log(`"${key.id}","${key.name}","${key.key_preview}","${key.status}","${key.permissions?.join(',') || ''}","${key.created}","${lastUsed}","${expiresAt}"`);
    });
    return;
  }
  ...
  keys.forEach((key, index) => {
    ...
    if (key.permissions && key.permissions.length > 0) {
      console.log(chalk.white('   🔓 Permissions:'), chalk.cyan(key.permissions.join(', ')));
    }
    
    console.log(chalk.white('   📅 Created:'), chalk.cyan(new Date(key.created).toLocaleDateString()));
    
    if (key.lastUsed) {
      console.log(chalk.white('   🕒 Last Used:'), chalk.cyan(new Date(key.lastUsed).toLocaleString()));
    } else {
      console.log(chalk.white('   🕒 Last Used:'), chalk.yellow('Never'));
    }
    
    if (key.expiresAt) {
      console.log(chalk.white('   ⏰ Expires:'), chalk.yellow(new Date(key.expiresAt).toLocaleString()));
    } else {
      console.log(chalk.white('   ⏰ Expires:'), chalk.green('Never'));
    }
  });
  ...
  console.log(chalk.white('  • Regenerate key: costkatana key regenerate <id>'));
  console.log(chalk.white('  • Deactivate key: costkatana key deactivate <id>'));
}
...
async function regenerateKey(keyId: string) {
  ...
  try {
    const response = await axios.patch(`${baseUrl}/api/api-keys/${keyId}/regenerate`, {}, {
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
...
function displayRegenerateKeyResult(result: any, options: any) {
  ...
  if (format === 'json') {
    ...
    console.log(`"${result.id}","${result.name}","${result.key}","${result.created}"`);
    return;
  }
  ...
  console.log(chalk.white('  • Update this key in your applications immediately'));
  console.log(chalk.white('  • Update ChatGPT Custom GPT Actions if applicable'));
...
async function deactivateKey(keyId: string) {
  ...
  try {
    const response = await axios.patch(`${baseUrl}/api/api-keys/${keyId}/deactivate`, {}, {
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
...
function displayDeactivateKeyResult(result: any, options: any) {
  ...
  if (format === 'json') {
    ...
    console.log(`"${result.id}","${result.name}","${result.status}","${result.deactivatedAt}"`);
    return;
  }
  ...
  console.log(chalk.white('📅 Deactivated:'), chalk.cyan(new Date().toLocaleString()));
  
  console.log(chalk.gray('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━'));
  console.log(chalk.yellow('💡 Note:'));
  console.log(chalk.white('  • This key is now inactive and cannot be used'));
  console.log(chalk.white('  • Applications using this key will receive 401 errors'));
