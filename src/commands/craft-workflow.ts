...
export function craftWorkflowCommand(program: Command) {
  const craftGroup = program
    ...
    .description('🔧 Compose a multi-step agent workflow and evaluate cost impact per step');
  ...
  craftGroup
    ...
    .option('--template <template>', 'Use predefined template (legal, marketing, research, custom)', 'custom')
    .option('--format <format>', 'Output format (table, json, csv)', 'table')
    .option('--export <path>', 'Export workflow data to file')
    .option('-v, --verbose', 'Show detailed workflow analysis')
  ...
  craftGroup
    ...
    .option('--include-optimization', 'Include optimization suggestions')
    .option('--include-alternatives', 'Include alternative workflow suggestions')
...
async function handleCraftWorkflow(_options: any) {
  ...
  console.log(chalk.white('  costkatana craft-workflow interactive              Interactive workflow builder'));
  console.log(chalk.white('  costkatana craft-workflow template <name>         Create from template'));
  console.log(chalk.white('  costkatana craft-workflow evaluate <file>         Evaluate existing workflow'));
  console.log(chalk.white('  costkatana craft-workflow export <file>           Export workflow'));
  console.log(chalk.white('  costkatana craft-workflow templates               List available templates'));
  
  console.log(chalk.gray('\nExamples:'));
  console.log(chalk.white('  costkatana craft-workflow interactive --name legal_summary_flow'));
  console.log(chalk.white('  costkatana craft-workflow template legal --customize'));
  console.log(chalk.white('  costkatana craft-workflow evaluate workflow.yaml'));
  console.log(chalk.white('  costkatana craft-workflow export workflow.json --format yaml'));
  ...
  console.log(chalk.white('  • customer-support - Customer support automation'));
...
async function buildWorkflowInteractively(options: any) {
  ...
  try {
    ...
    const response = await axios.post(`${baseUrl}/api/craft-workflow/interactive?${params}`, {
      name: options.name,
...
function displayWorkflowAnalysis(workflow: any, options: any) {
  ...
  if (format === 'json') {
    ...
    workflow.steps.forEach((step: any) => {
      console.log(`"${step.name}","${step.model}","${step.estimatedTokens}","${step.estimatedCost}","${step.estimatedLatency}","${step.description}"`);
    });
    return;
  }
  ...
  console.log(chalk.white('Estimated Total Cost:'), chalk.red(`$${workflow.totalEstimatedCost.toFixed(4)}`));
  console.log(chalk.white('Estimated Total Tokens:'), chalk.cyan(workflow.totalEstimatedTokens.toLocaleString()));
  console.log(chalk.white('Estimated Total Latency:'), chalk.yellow(`${workflow.totalEstimatedLatency}ms`));
  ...
  workflow.steps.forEach((step: any, index: number) => {
    const stepColor = step.estimatedCost > 0.1 ? chalk.red : 
                     step.estimatedCost > 0.05 ? chalk.yellow : chalk.green;
    ...
    // Cost Information
    console.log(chalk.white('   Estimated Cost:'), stepColor(`$${step.estimatedCost.toFixed(4)}`));
    console.log(chalk.white('   Estimated Tokens:'), chalk.cyan(step.estimatedTokens.toLocaleString()));
    console.log(chalk.white('   Estimated Latency:'), chalk.yellow(`${step.estimatedLatency}ms`));
    
    // Configuration
    if (step.configuration) {
      console.log(chalk.white('   Configuration:'));
      Object.entries(step.configuration).forEach(([key, value]: [string, any]) => {
  ...
  if (workflow.costBreakdown) {
    console.log(chalk.yellow.bold('\n💰 Cost Breakdown'));
    console.log(chalk.gray('─'.repeat(50)));
    console.log(chalk.white('Input Processing:'), chalk.cyan(`$${workflow.costBreakdown.inputProcessing.toFixed(4)}`));
    console.log(chalk.white('Content Generation:'), chalk.cyan(`$${workflow.costBreakdown.contentGeneration.toFixed(4)}`));
    console.log(chalk.white('Validation & QA:'), chalk.cyan(`$${workflow.costBreakdown.validation.toFixed(4)}`));
    console.log(chalk.white('Output Formatting:'), chalk.cyan(`$${workflow.costBreakdown.outputFormatting.toFixed(4)}`));
    console.log(chalk.white('Total:'), chalk.red(`$${workflow.costBreakdown.total.toFixed(4)}`));
  }
  ...
  if (workflow.performanceAnalysis) {
    console.log(chalk.yellow.bold('\n⚡ Performance Analysis'));
    console.log(chalk.gray('─'.repeat(50)));
    console.log(chalk.white('Sequential Execution:'), chalk.cyan(`${workflow.performanceAnalysis.sequential}ms`));
    console.log(chalk.white('Parallel Execution:'), chalk.cyan(`${workflow.performanceAnalysis.parallel}ms`));
    console.log(chalk.white('Optimization Potential:'), chalk.green(`${workflow.performanceAnalysis.optimizationPotential}%`));
    console.log(chalk.white('Bottleneck Steps:'), chalk.yellow(workflow.performanceAnalysis.bottlenecks.join(', ')));
  }
  ...
  if (workflow.optimizations && workflow.optimizations.length > 0) {
    ...
    workflow.optimizations.forEach((opt: any, index: number) => {
      ...
      console.log(chalk.gray(`   Potential Savings: $${opt.potentialSavings.toFixed(4)}`));
...
async function createWorkflowFromTemplate(templateName: string, options: any) {
  ...
  try {
    ...
    const response = await axios.post(`${baseUrl}/api/craft-workflow/template?${params}`, {
...
async function evaluateWorkflow(workflowFile: string, options: any) {
  ...
  try {
    ...
    const response = await axios.post(`${baseUrl}/api/craft-workflow/evaluate?${params}`, {
...
function displayWorkflowEvaluation(evaluation: any, options: any) {
  ...
  if (format === 'json') {
    ...
    console.log('Step,Model,Actual Tokens,Actual Cost,Estimated Tokens,Estimated Cost,Difference');
    evaluation.stepEvaluations.forEach((step: any) => {
      console.log(`"${step.name}","${step.model}","${step.actualTokens}","${step.actualCost}","${step.estimatedTokens}","${step.estimatedCost}","${step.difference}"`);
    });
    return;
  }
  ...
  console.log(chalk.yellow.bold('\n📊 Overall Results'));
  console.log(chalk.gray('─'.repeat(50)));
  console.log(chalk.white('Total Actual Cost:'), chalk.red(`$${evaluation.totalActualCost.toFixed(4)}`));
  console.log(chalk.white('Total Estimated Cost:'), chalk.yellow(`$${evaluation.totalEstimatedCost.toFixed(4)}`));
  console.log(chalk.white('Cost Accuracy:'), chalk.cyan(`${evaluation.costAccuracy}%`));
  console.log(chalk.white('Total Actual Tokens:'), chalk.cyan(evaluation.totalActualTokens.toLocaleString()));
  console.log(chalk.white('Total Estimated Tokens:'), chalk.cyan(evaluation.totalEstimatedTokens.toLocaleString()));
  console.log(chalk.white('Token Accuracy:'), chalk.cyan(`${evaluation.tokenAccuracy}%`));
  ...
  evaluation.stepEvaluations.forEach((step: any, index: number) => {
    const costAccuracyColor = step.costAccuracy >= 90 ? chalk.green : 
                             step.costAccuracy >= 70 ? chalk.yellow : chalk.red;
    ...
    // Cost Comparison
    console.log(chalk.white('   Actual Cost:'), chalk.red(`$${step.actualCost.toFixed(4)}`));
    console.log(chalk.white('   Estimated Cost:'), chalk.yellow(`$${step.estimatedCost.toFixed(4)}`));
    console.log(chalk.white('   Cost Accuracy:'), costAccuracyColor(`${step.costAccuracy}%`));
    
    // Token Comparison
    console.log(chalk.white('   Actual Tokens:'), chalk.cyan(step.actualTokens.toLocaleString()));
    console.log(chalk.white('   Estimated Tokens:'), chalk.cyan(step.estimatedTokens.toLocaleString()));
    console.log(chalk.white('   Token Accuracy:'), chalk.cyan(`${step.tokenAccuracy}%`));
    
    // Performance
    console.log(chalk.white('   Actual Latency:'), chalk.yellow(`${step.actualLatency}ms`));
    console.log(chalk.white('   Estimated Latency:'), chalk.yellow(`${step.estimatedLatency}ms`));
    
    // Quality Assessment
    if (step.qualityAssessment) {
      console.log(chalk.white('   Quality Score:'), chalk.cyan(step.qualityAssessment.score));
      console.log(chalk.white('   Quality Notes:'), chalk.gray(step.qualityAssessment.notes));
    }
  });
  ...
  if (evaluation.costBreakdown) {
    ...
    console.log(chalk.gray('─'.repeat(50)));
    Object.entries(evaluation.costBreakdown).forEach(([category, data]: [string, any]) => {
      console.log(chalk.white(`\n${category}:`));
  ...
  if (evaluation.alternatives && evaluation.alternatives.length > 0) {
    ...
    evaluation.alternatives.forEach((alt: any, index: number) => {
      console.log(chalk.white(`\n${index + 1}. ${alt.name}:`));
      console.log(chalk.gray(`   ${alt.description}`));
      console.log(chalk.gray(`   Estimated Cost: $${alt.estimatedCost.toFixed(4)}`));
...
async function exportWorkflow(workflowFile: string, options: any) {
  ...
  try {
    ...
    const response = await axios.post(`${baseUrl}/api/craft-workflow/export?${params}`, {
      workflowContent: workflowContent,
...
function displayWorkflowExport(exportData: any, options: any) {
  ...
  try {
    ...
    console.log(chalk.white('Workflow Name:'), chalk.cyan(exportData.workflowName));
    console.log(chalk.white('Export Format:'), chalk.cyan(format.toUpperCase()));
    console.log(chalk.white('Output File:'), chalk.green(outputPath));
    console.log(chalk.white('File Size:'), chalk.cyan(`${(exportData.content.length / 1024).toFixed(2)} KB`));
    
    if (exportData.metadata) {
      console.log(chalk.yellow.bold('\n📋 Export Metadata'));
      console.log(chalk.gray('─'.repeat(50)));
      console.log(chalk.white('Created:'), chalk.cyan(exportData.metadata.created));
      console.log(chalk.white('Version:'), chalk.cyan(exportData.metadata.version));
      console.log(chalk.white('Steps:'), chalk.cyan(exportData.metadata.steps));
      console.log(chalk.white('Estimated Cost:'), chalk.red(`$${exportData.metadata.estimatedCost.toFixed(4)}`));
    }
    
    if (exportData.includes) {
      console.log(chalk.yellow.bold('\n📚 Included Components'));
      console.log(chalk.gray('─'.repeat(50)));
      Object.entries(exportData.includes).forEach(([component, included]: [string, unknown]) => {
...
async function getAvailableTemplates(options: any) {
  ...
  try {
    ...
    const response = await axios.get(`${baseUrl}/api/craft-workflow/templates?${params}`, {
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json',
      },
...
function displayTemplates(templates: any, options: any) {
  ...
  if (format === 'json') {
    ...
    templates.templates.forEach((template: any) => {
      console.log(`"${template.name}","${template.category}","${template.steps}","${template.estimatedCost}","${template.description}"`);
    });
    return;
  }
  ...
  // Template Categories
  Object.entries(templates.categories).forEach(([category, categoryTemplates]: [string, any]) => {
    console.log(chalk.yellow.bold(`\n${category}`));
    console.log(chalk.gray('─'.repeat(50)));
    ...
    categoryTemplates.forEach((template: any) => {
      ...
      console.log(chalk.gray(`  Estimated Cost: $${template.estimatedCost.toFixed(4)}`));
      console.log(chalk.gray(`  Complexity: ${template.complexity}`));
      
      if (options.verbose && template.details) {
        console.log(chalk.gray(`  Use Cases: ${template.details.useCases.join(', ')}`));
        console.log(chalk.gray(`  Key Features: ${template.details.keyFeatures.join(', ')}`));
        console.log(chalk.gray(`  Requirements: ${template.details.requirements.join(', ')}`));
      }
    });
  });
  ...
  console.log(chalk.yellow.bold('\n🚀 Quick Start Guide'));
  console.log(chalk.gray('─'.repeat(50)));
  console.log(chalk.white('1. Choose a template:'), chalk.cyan('costkatana craft-workflow template legal'));
  console.log(chalk.white('2. Customize if needed:'), chalk.cyan('costkatana craft-workflow template legal --customize'));
  console.log(chalk.white('3. Evaluate cost impact:'), chalk.cyan('costkatana craft-workflow evaluate workflow.yaml'));
  console.log(chalk.white('4. Export workflow:'), chalk.cyan('costkatana craft-workflow export workflow.json --format yaml'));
