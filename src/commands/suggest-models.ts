...
export function suggestModelsCommand(program: Command) {
  const suggestGroup = program
    ...
    .description(
      '🧠 Suggest more cost-efficient models based on your prompt/task and usage patterns'
    );
  ...
  suggestGroup
    ...
    .option(
      '--task <task>',
      'Task type (summarization, qa, extraction, classification, generation, translation)',
      'auto'
    )
    .option(
      '--priority <priority>',
      'Priority (cost, quality, speed, carbon, balanced)',
      'balanced'
    )
    .option('--format <format>', 'Output format (table, json, csv)', 'table')
    .option('--export <path>', 'Export suggestions data to file')
...
async function handleSuggestModels(_options: any) {
  ...
  console.log(
    chalk.white(
      '  costkatana suggest-models prompt <prompt>     Suggest based on specific prompt'
    )
  );
  console.log(
    chalk.white(
      '  costkatana suggest-models task <taskType>     Suggest for specific task type'
    )
  );
  console.log(
    chalk.white(
      '  costkatana suggest-models usage               Suggest based on usage patterns'
    )
  );
  console.log(
    chalk.white(
      '  costkatana suggest-models cost                Suggest for cost optimization'
    )
  );
  console.log(
    chalk.white(
      '  costkatana suggest-models compare <m1> <m2>  Compare two specific models'
    )
  );
  ...
  console.log(
    chalk.white(
      '  costkatana suggest-models prompt "Summarize a 5-page contract"'
    )
  );
  console.log(
    chalk.white(
      '  costkatana suggest-models task summarization --priority cost'
    )
  );
  ...
  console.log(
    chalk.white('  costkatana suggest-models compare gpt-4 claude-3-sonnet')
  );
...
async function handleSuggestModelsByPrompt(prompt: string, options: any) {
  logger.info(
    `🧠 Suggesting models for prompt: "${prompt.substring(0, 50)}${prompt.length > 50 ? '...' : ''}"`
  );
...
async function getSuggestionsByPrompt(prompt: string, options: any) {
  ...
  try {
    ...
    const response = await axios.post(
      `${baseUrl}/api/suggest-models/prompt?${params}`,
      {
        prompt: prompt,
...
function displayModelSuggestions(suggestions: any, options: any) {
  ...
  if (format === 'json') {
    ...
    console.log(
      'Rank,Model,Provider,Cost/Token,Quality Score,Speed Score,Carbon Score,Recommendation'
    );
    suggestions.recommendations.forEach((rec: any, index: number) => {
      console.log(
        `"${index + 1}","${rec.model}","${rec.provider}","${rec.costPerToken}","${rec.qualityScore}","${rec.speedScore}","${rec.carbonScore}","${rec.recommendation}"`
      );
    });
    return;
  }
  ...
  if (suggestions.taskAnalysis) {
    ...
    console.log(
      chalk.white('Detected Task:'),
      chalk.cyan(suggestions.taskAnalysis.detectedTask)
    );
    console.log(
      chalk.white('Confidence:'),
      chalk.cyan(`${(suggestions.taskAnalysis.confidence * 100).toFixed(1)}%`)
    );
    console.log(
      chalk.white('Complexity:'),
      chalk.cyan(suggestions.taskAnalysis.complexity)
    );
    console.log(
      chalk.white('Estimated Tokens:'),
      chalk.cyan(suggestions.taskAnalysis.estimatedTokens.toLocaleString())
    );
  }
  ...
  suggestions.recommendations.forEach((rec: any, index: number) => {
    const rankColor =
      index === 0 ? chalk.green : index === 1 ? chalk.yellow : chalk.blue;
    ...
    console.log(
      chalk.white('   Cost/Token:'),
      chalk.cyan(`$${rec.costPerToken.toFixed(6)}`)
    );

    // Scores
    const qualityColor =
      rec.qualityScore >= 0.8
        ? chalk.green
        : rec.qualityScore >= 0.6
          ? chalk.yellow
          : chalk.red;
    console.log(
      chalk.white('   Quality Score:'),
      qualityColor(rec.qualityScore.toFixed(3))
    );
    ...
    const speedColor =
      rec.speedScore >= 0.8
        ...
        : rec.speedScore >= 0.6
          ...
          : chalk.red;
    console.log(
      chalk.white('   Speed Score:'),
      speedColor(rec.speedScore.toFixed(3))
    );
    ...
    const carbonColor =
      rec.carbonScore <= 0.3
        ...
        : rec.carbonScore <= 0.6
          ...
          : chalk.red;
    console.log(
      chalk.white('   Carbon Score:'),
      carbonColor(rec.carbonScore.toFixed(3))
    );
    ...
    if (rec.costAnalysis) {
      console.log(
        ...
        chalk.cyan(`$${rec.costAnalysis.estimatedCost.toFixed(4)}`)
      );
      console.log(
        chalk.white('   Cost Savings:'),
        chalk.green(`${rec.costAnalysis.costSavings}%`)
      );
    }
    ...
    console.log(
      ...
      chalk.cyan(rec.recommendation)
    );

    // Rank indicator
    console.log(
      rankColor(
        `   ${index === 0 ? '🥇' : index === 1 ? '🥈' : '🥉'} ${index === 0 ? 'Best Choice' : index === 1 ? 'Good Alternative' : 'Consider'}`
      )
    );
  });
  ...
  if (suggestions.costComparison) {
    ...
    console.log(
      ...
      chalk.cyan(suggestions.costComparison.currentModel)
    );
    console.log(
      chalk.white('Current Cost:'),
      chalk.cyan(`$${suggestions.costComparison.currentCost.toFixed(4)}`)
    );
    console.log(
      chalk.white('Recommended Cost:'),
      chalk.green(`$${suggestions.costComparison.recommendedCost.toFixed(4)}`)
    );
    console.log(
      chalk.white('Potential Savings:'),
      chalk.green(`$${suggestions.costComparison.potentialSavings.toFixed(4)}`)
    );
    console.log(
      chalk.white('Savings Percentage:'),
      chalk.green(`${suggestions.costComparison.savingsPercentage}%`)
    );
  }
  ...
  if (suggestions.carbonAnalysis) {
    ...
    console.log(chalk.gray('─'.repeat(50)));
    console.log(
      chalk.white('Current Carbon Impact:'),
      chalk.cyan(`${suggestions.carbonAnalysis.currentCarbon}g CO2`)
    );
    console.log(
      chalk.white('Recommended Carbon Impact:'),
      chalk.green(`${suggestions.carbonAnalysis.recommendedCarbon}g CO2`)
    );
    console.log(
      chalk.white('Carbon Reduction:'),
      chalk.green(`${suggestions.carbonAnalysis.carbonReduction}%`)
    );
  }
...
async function getSuggestionsByTask(taskType: string, options: any) {
  ...
  try {
    ...
    const response = await axios.get(
      `${baseUrl}/api/suggest-models/task?${params}`,
      {
        headers: {
          Authorization: `Bearer ${apiKey}`,
          'Content-Type': 'application/json',
...
async function handleSuggestModelsByUsage(options: any) {
  logger.info(
    `🧠 Suggesting models based on usage patterns for range: ${options.range || '30d'}`
  );
...
async function getSuggestionsByUsage(range: string, options: any) {
  ...
  try {
    ...
    const response = await axios.get(
      `${baseUrl}/api/suggest-models/usage?${params}`,
      {
        headers: {
          Authorization: `Bearer ${apiKey}`,
          'Content-Type': 'application/json',
...
async function getModelComparison(
  ...
  try {
    ...
    const response = await axios.get(
      `${baseUrl}/api/suggest-models/compare?${params}`,
      {
        headers: {
          Authorization: `Bearer ${apiKey}`,
          'Content-Type': 'application/json',
...
function displayModelComparison(comparison: any, options: any) {
  ...
  if (format === 'json') {
    ...
    Object.entries(comparison.metrics).forEach(
      ([metric, data]: [string, any]) => {
        console.log(
          `"${metric}","${data.model1}","${data.model2}","${data.difference}","${data.recommendation}"`
        );
      }
    );
    return;
  ...
  Object.entries(comparison.metrics).forEach(
    ([metric, data]: [string, any]) => {
      ...
      console.log(
        chalk.gray(`  ${comparison.model1}:`),
        model1Color(data.model1)
      );
      console.log(
        chalk.gray(`  ${comparison.model2}:`),
        model2Color(data.model2)
      );
      ...
      if (data.recommendation) {
        console.log(
          ...
          chalk.cyan(data.recommendation)
        );
      }
    }
  );
  ...
  if (comparison.costAnalysis) {
    ...
    console.log(chalk.gray('─'.repeat(50)));
    console.log(
      chalk.white(`${comparison.model1} Cost:`),
      chalk.cyan(`$${comparison.costAnalysis.model1Cost.toFixed(6)}/token`)
    );
    console.log(
      chalk.white(`${comparison.model2} Cost:`),
      chalk.cyan(`$${comparison.costAnalysis.model2Cost.toFixed(6)}/token`)
    );
    console.log(
      chalk.white('Cost Difference:'),
      chalk.green(`${comparison.costAnalysis.costDifference}%`)
    );
    console.log(
      chalk.white('Savings:'),
      chalk.green(`$${comparison.costAnalysis.savings.toFixed(4)}/1K tokens`)
    );
  }
  ...
  if (comparison.performanceAnalysis) {
    ...
    console.log(chalk.gray('─'.repeat(50)));
    console.log(
      chalk.white(`${comparison.model1} Speed:`),
      chalk.cyan(`${comparison.performanceAnalysis.model1Speed}ms`)
    );
    console.log(
      chalk.white(`${comparison.model2} Speed:`),
      chalk.cyan(`${comparison.performanceAnalysis.model2Speed}ms`)
    );
    console.log(
      chalk.white('Speed Difference:'),
      chalk.green(`${comparison.performanceAnalysis.speedDifference}%`)
    );
  }
  ...
  if (comparison.qualityAnalysis) {
    ...
    console.log(chalk.gray('─'.repeat(50)));
    console.log(
      chalk.white(`${comparison.model1} Quality:`),
      chalk.cyan(comparison.qualityAnalysis.model1Quality)
    );
    console.log(
      chalk.white(`${comparison.model2} Quality:`),
      chalk.cyan(comparison.qualityAnalysis.model2Quality)
    );
    console.log(
      chalk.white('Quality Difference:'),
      chalk.green(`${comparison.qualityAnalysis.qualityDifference}%`)
    );
  }
  ...
  if (comparison.recommendation) {
    ...
    console.log(
      chalk.gray(`Reasoning: ${comparison.recommendation.reasoning}`)
    );
